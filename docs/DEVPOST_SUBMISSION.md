...
- **Vertex AI** — Gemini 2.5 Flash, called through the Google Agent Development Kit (ADK) for
  both agents' reasoning
- **Vertex AI Agent Engine / Google ADK** — the agent framework itself
- **Cloud Run** — two deployed services: the Streamlit frontend and `mcp-grafana` (the MCP
  server), including service-to-service auth via Cloud Run's own IAM (Google-issued ID tokens)
- **Cloud Build** — builds the frontend's container image from source on every deploy
- **Artifact Registry** — stores the built container images
- **IAM** — a dedicated runtime service account with scoped `run.invoker` and `aiplatform.user`
  roles, rather than broad permissions
- **Cloud Logging** — used throughout for debugging deployed service behavior

## Please list all other tools or products you used in your project.

- **Grafana Cloud** (Mimir, Loki, Dashboards, Alerting) — the partner track's core service, and
  the system's actual data plane
- **Grafana MCP server** (`grafana/mcp-grafana`) — Grafana's own open-source MCP implementation
- **Model Context Protocol (MCP)** — the protocol connecting the agents to Grafana
- **Streamlit** — the frontend
- **Python** — agents, data generator, frontend
- **Docker** — containerizing the frontend for Cloud Run; also runs `mcp-grafana` locally in dev
- **Prometheus remote_write protocol** — how synthetic metrics reach Mimir
- **Plotly** — charts in the frontend
- **GitHub** — source control and the public repo
